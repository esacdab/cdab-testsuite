...
echo "DOCKER COMMAND: docker run --workdir /res -v ${PWD}:/res${eodata_volume_option} --env DOWNLOAD_ORIGIN=${download_origin} \"${docker_image}\" mono $MONO_OPTIONS /usr/lib/cdab-client/bin/cdab-testtools.exe --target_name \"${target_name}\" --target_url \"${target_url}\" --target_credentials \"${target_credentials_display}\" --testsite_name \"${test_site}\" -lf \"${load_factor}\" --conf /res/config.yaml -v TS11" > cdab.stderr
docker run --workdir /res -v ${PWD}:/res${eodata_volume_option} --env DOWNLOAD_ORIGIN=${download_origin} "${docker_image}" mono $MONO_OPTIONS /usr/lib/cdab-client/bin/cdab-testtools.exe --target_name "${target_name}" --target_url "${target_url}" --target_credentials "${target_credentials}" --testsite_name "${test_site}" -lf "${load_factor}" --conf /res/config.yaml -v TS11 > cdab.stdout 2>> cdab.stderr
res=$?
if [ $res -ne 0 ]
then
    cat cdab.stdout
